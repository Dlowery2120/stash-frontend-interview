...
export function SearchBar() {
  ...
  const [nearbyCity, setNearbyCity] = React.useState<string | null>(null)
  ...
  const filteredHotelOptions = hotelOptions.filter(option =>
    query.length >= 3 &&
                ...
                <CommandList>
                  ...
                  <CommandGroup heading="Location">
                    ...
                    >
                      ...
                      <Check className={cn('ml-auto h-4 w-4', selectedOption?.value === locationOption.value ? 'opacity-100' : 'opacity-0')} />
                    </CommandItem>
                  </CommandGroup>
                    ...
                    <CommandGroup heading="Stash Partner Hotels">
                      {filteredHotelOptions.map((option) => (
                        ...
                        >
                          ...
                          <Check className={cn('ml-auto h-4 w-4', selectedOption?.value === option.value ? 'opacity-100' : 'opacity-0')} />
                        </CommandItem>
                      ))}
                    </CommandGroup>
                  )}
            ...
            <PopoverContent className="w-56 p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="ghost" onClick={() => setAdults((prev) => Math.max(1, prev - 1))}>–</Button>
                  <span>{adults}</span>
                  <Button variant="ghost" onClick={() => setAdults(adults + 1)}>+</Button>
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span>Children</span>
                <div className="flex items-center gap-2">
                  <Button variant="ghost" onClick={() => setChildren((prev) => Math.max(0, prev - 1))}>–</Button>
                  <span>{children}</span>
                  <Button variant="ghost" onClick={() => setChildren(children + 1)}>+</Button>
                </div>
              </div>
            </PopoverContent>
          </Popover>
